...
public class BinaryExpressionBuilder implements ExpressionBuilder {
    ...
    public Expression build(final List<String> expressionTokens, final SourceLine sourceLine) {
        ...
        final BinaryOperator operator = BinaryOperator.of(expressionTokens.get(1)).orElseThrow(() -> {
            throw new JavammLineSyntaxError("Unsupported binary operator: " + expressionTokens.get(1), sourceLine);
        });
