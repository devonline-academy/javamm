...
public class ContinueOperationReader extends AbstractOperationReader<ContinueOperation> {
    ...
    protected void validate(final SourceLine sourceLine) {
        if (sourceLine.getTokenCount() > 1) {
            throw new JavammLineSyntaxError(format("'%s' expected only", CONTINUE), sourceLine);
        }
    }

    @Override
    protected ContinueOperation get(final SourceLine sourceLine, final ListIterator<SourceLine> iterator) {
